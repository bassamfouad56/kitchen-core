...
async function seedProductionAbout() {
  try {
    ...
    if (existingFounder) {
      ...
      console.log("✓ Created founder: Eng. Esam Odeh");
    }

    // Check team members count
    const teamCount = await prisma.teamMember.count();
    if (teamCount >= 6) {
      console.log(`✓ Team members already exist (${teamCount} members)`);
    } else {
      // Delete any existing team members first

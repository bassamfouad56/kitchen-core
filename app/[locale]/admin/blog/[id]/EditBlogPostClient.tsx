...
export default function EditBlogPostClient({ id }: { id: string }) {
  ...
  const fetchPost = useCallback(async () => {
    try {
      ...
      setSeoKeywords(data.seoKeywords.length > 0 ? data.seoKeywords : [""]);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-8 flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-light">Update blog post</p>
          </div>
          <LanguageSwitcher />
        </div>
        ...
        {/* View Count */}
        <div className="mb-6 bg-background-card border border-gray-dark p-4">
          <div className="flex items-center gap-4">
            <span className="text-gray-light">
              {t("views")}: <span className="text-white">{post.views}</span>
            </span>
          </div>
        </div>

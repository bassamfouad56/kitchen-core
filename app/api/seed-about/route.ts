...
export async function POST(request: Request) {
  try {
    ...
    if (secret !== "seed-production-about-2025") {
    ...
    if (teamCount >= 6) {
      return NextResponse.json({
        ...
        founder: existingFounder ? "exists" : "created",
        teamMembers: teamCount,
      });
    }

    // Delete and recreate team members
    ...
    return NextResponse.json(
      { error: "Failed to seed production database", details: String(error) },
      { status: 500 },
    );
  } finally {
